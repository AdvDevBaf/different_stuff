...
class Gui(Toplevel):
    ...
    def save_to_csv(self,tittle, date_of_publish, comments):
        ...
        print('ok')
        return {}
    def get_videos_to_excel(self,videoId):
        df = pd.DataFrame({'Video': [videoId]})

        # Create a Pandas Excel writer using XlsxWriter as the engine.
        writer = pd.ExcelWriter('video_iids.xlsx', engine='xlsxwriter')
    ...
    def get_comments_to_excel(self,comments, date_of_publish,ch_id):
        df = pd.DataFrame({'Comments': comments, 'Date_of_publish':date_of_publish, 'Ch_id':ch_id})

        # Create a Pandas Excel writer using XlsxWriter as the engine.
        writer = pd.ExcelWriter('comments_in_cycle.xlsx', engine='xlsxwriter')
    ...
    def get_videos(self, uploads):
        ...
        for name in uploads:
            ...
            for i in range(0, len(last_upload['items'])):
                ...
                self.get_videos_to_excel(videoId)
                print('ne token')
            #try:
        ...
        for vid in videoId:
            ...
            try:
                for i in range(1, len(comms['items'])):
                    ...
                    self.get_comments_to_excel(comments, date_of_publish, ch_id)
                    print('ne token')
    ...
    def start(self):
        ...
        video_id = self.get_videos(uploads)
        comments, date_of_publish, tittle = self.get_comments(video_id)
